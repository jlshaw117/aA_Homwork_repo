...
def below_average_years
  ...
  Movie.select('COUNT(*) AS bad_movies', :yr).where('score < 5').group(:yr).order('bad_movies')
end
...
def pulp_fiction_actors
  ...
  Actor.select('actors.*').joins(:movies).where('title = ?', 'Pulp Fiction')
end
...
def uma_movies
  ...
  Movie.select('movies.id, title, yr').joins(:actors).where('name = ?', 'Uma Thurman')
end
